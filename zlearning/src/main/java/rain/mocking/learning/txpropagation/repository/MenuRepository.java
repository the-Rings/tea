package rain.mocking.learning.txpropagation.repository;
...
import rain.mocking.learning.txpropagation.domain.MenuItem;
...
public class MenuRepository {
  ...
  public int insertItemWithNamedParameter(MenuItem item) {
    String sql =
        "insert into t_menu (name, size, price, create_time, update_time) values "
    ...
    sqlParameterSource.addValue(
        "price", item.getPrice().multiply(BigDecimal.valueOf(100)).longValue());
        ...
        jdbcTemplate.update(
            con -> {
              ...
              preparedStatement.setLong(
                  3, item.getPrice().multiply(BigDecimal.valueOf(100)).longValue());
  ...
  public int insertItemsWithNamedParameter(List<MenuItem> items) {
    ...
    int[] count =
        namedParameterJdbcTemplate.batchUpdate(sql, SqlParameterSourceUtils.createBatch(items));
    return Arrays.stream(count).sum();
  }
